...
// 철산SOS — 단일 파일 랜딩 페이지
// Tailwind 기반, 외부 라이브러리 없이 동작
// 사용법: 기본 export 컴포넌트를 루트에 렌더링하면 됩니다.
// TODO: 아래 KAKAO_CHAT_LINK 를 실제 비즈니스 채널 링크로 바꿔주세요.

const KAKAO_CHAT_LINK = "http://pf.kakao.com/_xdmQxkn/chat"; // <- 여기를 실제 채널로 교체
    ...
    <main className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <header className="sticky top-0 z-50 border-b bg-white/80 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <a href={KAKAO_CHAT_LINK} target="_blank" rel="noreferrer" className="rounded-xl bg-amber-500 px-4 py-2 text-sm font-semibold text-white shadow transition hover:bg-amber-600">카카오채널로 문의하기</a>
          </div>
        </div>
      </header>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="mx-auto grid max-w-6xl gap-8 px-4 py-16 md:grid-cols-2 md:gap-12 md:py-24">
          <div>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              ...
              <a href="#how" className="rounded-2xl border border-gray-300 px-6 py-3 text-sm font-semibold text-gray-700 transition hover:bg-white">이용방법 보기</a>
            </div>
          ...
          <div className="">
            <div className="relative mx-auto max-w-md overflow-hidden rounded-2xl border bg-white p-4 shadow-sm md:max-w-none">
              ...
              <div className="mt-4 flex items-center justify-between rounded-xl bg-gray-900 p-3 text-white">
                ...
                <a href={KAKAO_CHAT_LINK} target="_blank" rel="noreferrer" className="rounded-lg bg-white px-3 py-2 text-xs font-semibold text-gray-900">지금 문의</a>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="how" className="border-t bg-white">
        <div className="mx-auto max-w-6xl px-4 py-16">
            ...
            {[1,2,3].map((n, i) => (
              <li key={i} className="rounded-2xl border bg-gray-50 p-5 shadow-sm">
                <div className="flex items-center gap-3">
                  <span className="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-amber-500 text-sm font-bold text-white">{n}</span>
          ...
          <div className="mt-6 text-xs text-gray-500">* 저희는 연결 중개 서비스입니다. 작업·결제·AS는 기사님과 고객이 직접 진행합니다.</div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-6xl px-4 py-16">
          ...
          <div className="mt-6 grid grid-cols-2 gap-3 text-sm sm:grid-cols-3 md:grid-cols-4">
            {[
              ...
              <div key={idx} className="rounded-xl border bg-white px-4 py-3 text-center shadow-sm">{label}</div>
            ))}
          </div>
        ...
        <div className="mx-auto max-w-6xl px-4 py-16">
          ...
          <div className="mt-6 grid grid-cols-2 gap-3 text-sm sm:grid-cols-3 md:grid-cols-4">
            {["철산", "광명", "구로", "독산", "철산역 인근", "광명사거리", "구로디지털", "가산/독산 경계"].map((a, i) => (
              <div key={i} className="rounded-xl border bg-gray-50 px-4 py-3 text-center">{a}</div>
            ))}
          </div>
        </div>
      </section>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <div className="rounded-2xl border bg-white p-5 shadow-sm">
              ...
              <p className="mt-3 text-sm text-gray-700 leading-6">
                본 서비스는 <b>지역 기사 연결 중개 서비스</b>입니다. 작업 품질, A/S, 금액 등 모든 책임은 <b>작업을 수행한 기사님</b>에게 있으며, 비용 및 작업 진행은 <b>고객과 기사님이 직접 협의</b>하여 결정합니다.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-6xl px-4 py-16">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            {[
              {
                q: "결제는 어디에 하나요?",
                a: "결제는 방문 기사님과 직접 진행합니다. 저희는 중개자로서 작업·결제 과정에 참여하지 않습니다.",
              },
              {
                q: "가격은 어떻게 되나요?",
              ...
              {
                q: "내 전화번호가 노출되나요?",
                a: "아니요. 카카오채널 채팅으로 안전하게 접수되며, 필요한 경우에만 기사님과 연결됩니다.",
              },
              {
                q: "AS는 누가 책임지나요?",
                a: "작업을 진행한 기사님이 책임집니다. 필요 시 해당 기사님과 재연결을 도와드립니다.",
              },
            ].map((item, idx) => (
              <div key={idx} className="rounded-2xl border bg-gray-50 p-5 shadow-sm">
                <div className="font-semibold">Q. {item.q}</div>
                <div className="mt-2 text-sm text-gray-700 leading-6">A. {item.a}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="border-t bg-amber-50">
        <div className="mx-auto flex max-w-6xl flex-col items-center gap-4 px-4 py-14 text-center">
          ...
          <p className="max-w-2xl text-sm text-gray-700">주소 · 증상 · 사진 1~2장만 보내주세요. 빠르면 10~20분 내 출동 안내가 가능합니다.</p>
          <a href={KAKAO_CHAT_LINK} target="_blank" rel="noreferrer" className="rounded-2xl bg-gray-900 px-6 py-3 text-sm font-semibold text-white shadow-md transition hover:-translate-y-0.5 hover:bg-black">카카오채널로 문의하기</a>
        </div>
      </section>
      ...
      <footer className="border-t bg-white">
        <div className="mx-auto grid max-w-6xl gap-6 px-4 py-10 md:grid-cols-2">
          <div>
            <div className="text-lg font-bold">철산SOS</div>
            <div className="mt-1 text-sm text-gray-500">생활출동 중개 플랫폼 · 고객과 기사 연결 서비스</div>
            <div className="mt-3 text-xs text-gray-400">
              ⓘ 본 사이트의 모든 서비스는 중개 성격이며, 작업/비용/AS는 방문 기사님과 고객이 직접 진행합니다.
            </div>
          </div>
        ...
        <div className="border-t py-6 text-center text-xs text-gray-400">© {new Date().getFullYear()} CheolsanSOS. All rights reserved.</div>
      </footer>
    </main>
  );
}
